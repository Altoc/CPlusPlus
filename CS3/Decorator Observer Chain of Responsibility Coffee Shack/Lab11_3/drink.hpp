...
public:
   ...
   virtual void handleRequest (char toppingChoice, std::string result) {
      if (successor_ != nullptr)
	 successor_ -> handleRequest(toppingChoice, result);
   }
private:
   Barista *successor_;
   std::string drink_;
...
public:
   JuniorBarista(Barista *successor=nullptr):
                     Barista(successor){}
   void handleRequest (char toppingChoice, std::string result) override {
      if (toppingChoice == 'n')
	 std::cout << "Junior Barista makes "<< result << std::endl;
      else
	 Barista::handleRequest(toppingChoice, result);
   }
};
...
public:
   ...
   void handleRequest (char toppingChoice, std::string result) override {
      if (toppingChoice != 'f')
	 std::cout << "Senior Barista makes "<< result << std::endl;
      else
	 Barista::handleRequest(toppingChoice, result);
   }
};
...
public:
   ...
   void handleRequest (char toppingChoice, std::string result) override {
	if(toppingChoice == 'f')
		std::cout << "Manager Barista makes "<< result << std::endl;
	else
		Barista::handleRequest(toppingChoice, result);
   }
};