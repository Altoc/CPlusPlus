...
public:
   ...
   void makeMove(int player) {
	int compRollArr[6];
	for(auto compDieRollValue : compRollArr){
		std::cerr<<"Setting comp die value"<<std::endl;
		compDieRollValue = 0;
	}
	std::cout<<"Computer Rolled: "<<compRollArr[0]<<std::endl;
	playerWon_ = player;
   }

   void printWinner(){
	std::cout<<"printWinner()"<<std::endl;
   }

private:
   static const int numPlayers_ = 2; 	// number of players
   static const int movesCount_ = 3;		// number of rounds played
   int playerWon_ = 0;		// 0 = computer, 1 = human

   static const int numDice_ = 5;	// number of dice
   static const int minDiceScore_ = 1;  // number of min single die score
   static const int maxDiceScore_ = 6;  // number of max single die score
   int currRounds_ = 0;	// number of rounds possible   
};
